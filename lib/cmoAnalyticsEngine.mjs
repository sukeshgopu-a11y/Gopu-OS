...
async function collectOne(client, row, { dryRun = false } = {}) {
  ...
  if (String(row.publish_status || "").toLowerCase() !== "published") {
    await writeAudit(client, row, "analytics_failed", "Production analytics skipped because content is not published.", { risk_level: "Medium" });
    return { ok: false, status: "not_published", message: "Production analytics only runs for publish_status=published rows.", content_history: row };
  }
...
export async function runCmoAnalyticsEngine(options = {}) {
  ...
  if (!options.contentHistoryId && !options.runId) {
    query = dryRun
      ? query.eq("approval_status", "approved").eq("publish_status", "queued")
